...
import { AiGameState, Card, GameHistoryEntry, HeroName, WeatherType, Winner } from '../types';
import {
    ELEMENTS,
    ABILITIES,
    HAND_SIZE,
...
import { createFusionCard } from '../services/fusionService';
import {
...
export const useGameLogic = () => {
    ...
    const playCard = useCallback((cardId: string) => {
        ...
        const aiDecision = chooseCard(playedPlayerCard, aiHand, gameState);
        const playedAiCard = aiDecision.card;
        setAiCard(playedAiCard);

        const remainingAiHand = aiHand.filter(c => !aiDecision.consumedCardIds.includes(c.id));
        ...
        const mechanicMessages = [...mechanicOutcome.messages];
        if (aiDecision.createdByFusion && aiDecision.fusionSources) {
            const fusionNames = aiDecision.fusionSources
                .map(source => `${source.element}-${source.wert}`)
                .join(' + ');
            mechanicMessages.unshift(
                `Fusion: Die KI kombiniert ${fusionNames} zu ${playedAiCard.element}-${playedAiCard.wert}.`
            );
        }
        const combinedStatus = mechanicMessages.length > 0
            ? `${roundLabel} ${mechanicMessages.join(' ')}`
            : roundLabel;
